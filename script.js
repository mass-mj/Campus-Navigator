...
if (searchForm) {
    searchForm.addEventListener('submit', function(e) {
        ...
        if (searchInput.value.trim() === '') {
            ...
            window.location.href = '../navpages/college-search.php?q=' + encodeURIComponent(searchInput.value);
        }
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (searchForm) {
        searchForm.addEventListener('submit', function(e) {
            // The form now has proper method="GET" and action attributes,
            // so let it handle the submission naturally without preventing default
            // or manually redirecting
        });
    }
    
    // Search input validation - highlight if empty on focus out
